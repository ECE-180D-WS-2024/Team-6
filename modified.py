...
while True:
    ...
    # Calculate the distance between the camera and the pink object
    if center_pink:
        distance_to_camera = calculate_distance(center_pink, (frame.shape[1] // 2, frame.shape[0] // 2))
        cv2.putText(frame, f"Pink-Cam Distance: {distance_to_camera:.2f} pixels", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)

    # Calculate the distance between the pink and green objects
    if center_pink and center_green:
        distance_pink_to_green = calculate_distance(center_pink, center_green)
        cv2.putText(frame, f"Pink-Green Distance: {distance_pink_to_green:.2f} pixels", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
