...
while True:
    ...
    distance_blue_yellow = None
    distance_pink_green = None
    ...
    if center_blue and center_yellow:
        ...
        cv2.putText(frame, f"Blue-Yellow: {distance_blue_yellow:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    ...
    if center_pink and center_green:
        ...
        cv2.putText(frame, f"Pink-Green: {distance_pink_green:.2f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    # Calculate and display the ratio of the distances
    if distance_blue_yellow is not None and distance_pink_green is not None and distance_blue_yellow != 0:
        ratio = distance_pink_green / distance_blue_yellow
        cv2.putText(frame, f"Ratio: {ratio:.2f}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        # Check if the ratio is greater than 1 or not
        if ratio > 1:
            status = "Fail"
            color = (0, 0, 255)  # Red color for Fail
        else:
            status = "Success"
            color = (0, 255, 0)  # Green color for Success
        
        # Display and print the status
        cv2.putText(frame, status, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
